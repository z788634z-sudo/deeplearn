...
class DNALLMModel(nn.Module):
    ...
    def process_dna_embeddings(
        ...
        def _empty_result() -> List[torch.Tensor]:
            empty_tensor = torch.zeros(
                (0, self.text_hidden_size),
                device=self.dna_projection.weight.device,
                dtype=self.dna_projection.weight.dtype,
            )
            return [empty_tensor.clone() for _ in range(batch_size)]

        input_ids = dna_tokenized.get("input_ids")
        attention_mask = dna_tokenized.get("attention_mask")

        if input_ids is None or attention_mask is None or input_ids.size(0) == 0:
        ...
        with torch.no_grad():
            ...
            if self.dna_is_evo2 and self.dna_embedding_layer is not None:  # Evo2 model
                # Call Evo2 once with the full batch to obtain embeddings
        ...
        final_result: List[torch.Tensor] = []
